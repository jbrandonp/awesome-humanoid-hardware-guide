...
export function ClinicalTicker() {
  const [alerts] = useState<
    { id: string; msg: string; type: 'epi' | 'stock' }[]
  >([
    {
      id: '1',
      msg: 'Alerte Flash: Pic anormal de cas de Paludisme détecté en 48h (+35 cas).',
      type: 'epi',
    },
    {
      id: '2',
      msg: 'Stock Critique: Artemether < 15 unités. Prévoyez une rupture imminente.',
      type: 'stock',
    },
  ]);
    ...
    <div className="fixed bottom-0 left-0 right-0 h-10 bg-slate-950 border-t border-medical-border flex items-center overflow-hidden z-50">
      <div className="flex animate-marquee whitespace-nowrap items-center h-full px-4">
        {alerts.map((alert) => (
          <span
            key={alert.id}
            className="flex items-center text-sm font-medium mr-16 text-slate-300"
          >
            {alert.type === 'epi' ? (
