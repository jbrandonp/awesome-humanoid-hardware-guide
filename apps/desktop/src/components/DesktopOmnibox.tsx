...
export const DesktopOmnibox = React.forwardRef<HTMLInputElement, unknown>((props, ref) => {
               ...
               return (
                 ...
                 >
                   <div className="flex items-center gap-3">
                     {item.type === 'icd10' && <Stethoscope className={isSelected ? 'text-white' : 'text-blue-400'} />}
                     {item.type === 'medication' && <Pill className={isSelected ? 'text-white' : 'text-emerald-400'} />}
                     {item.type === 'lab' && <Activity className={isSelected ? 'text-white' : 'text-purple-400'} />}
                     <div>
                       <div className="font-semibold text-lg">
                         {item.type === 'icd10' ? `[${item.code}] ` : ''} {item.name}
                       </div>
                       {item.dosage && (
                         <div className={`text-sm mt-1 ${isSelected ? 'text-blue-100' : 'text-slate-400'}`}>
                           {item.dosage}
                         </div>
                       )}
                     </div>
                   </div>
                   {isSelected && <span className="text-xs bg-white/20 px-2 py-1 rounded">Appuyez sur Entrée ↵</span>}
                 </button>
               );
            })}
         </div>
