  ...
  try {
    ...
    for (const task of queue) {
      ...
      try {
        ...
        // Rejeter définitivement les erreurs client 4xx (sauf 408 Timeout et 429 Too Many Requests).
        // Les erreurs 5xx (Serveur) et pannes réseau sont transitoires et seront réessayées.
        if (
          status &&
          status >= 400 &&
          status < 500 &&
          ...
          console.error(
            `[OS Background Worker] Erreur client permanente (${status}) pour la tâche ${task.transactionId}. Tâche rejetée pour ne pas inonder le serveur.`,
          );
          continue;
        }
