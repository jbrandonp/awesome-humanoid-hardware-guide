...
export function PatientDashboard({ patientName, heartRateHistory, temperatureHistory }: PatientDashboardProps) {

  // Formatage des données pour Victory
  const hrData = useMemo(() => heartRateHistory.map(hr => ({
     x: new Date(hr.recordedAt),
     y: hr.value
  })), [heartRateHistory]);

  const tempData = useMemo(() => temperatureHistory.map(t => ({
      ...
      <View style={styles.chartContainer}>
        ...
        <VictoryChart theme={VictoryTheme.material} height={200} padding={{ top: 20, bottom: 40, left: 40, right: 20 }}>
           ...
           <VictoryLine
             ...
             data={hrData}
           />
           <VictoryScatter data={hrData} size={4} style={{ data: { fill: "#c43a31" } }} />
        </VictoryChart>
      </View>
      ...
      <View style={styles.chartContainer}>
        ...
        <VictoryChart theme={VictoryTheme.material} height={200} padding={{ top: 20, bottom: 40, left: 40, right: 20 }}>
           ...
           <VictoryLine
             ...
             data={tempData}
           />
           <VictoryScatter data={tempData} size={4} style={{ data: { fill: "#3498db" } }} />
        </VictoryChart>
      </View>
    </View>
  );
