...
export function SmartPenCanvas() {
  ...
  useEffect(() => {
    ...
    if (isConnected) {
      ...
      try {
        ...
        console.error(
          '[SmartPen FATAL] Le SDK du stylo Bluetooth a crashé.',
          sdkError,
        );
        setHardwareError(
          'Le pilote matériel du stylo connecté a cessé de fonctionner.',
        );
  ...
  const processRawHardwareCoordinate = (event: SmartPenRawEvent) => {
    try {
      ...
      console.error(
        '[SmartPen] Erreur de traitement du vecteur SVG',
        processingError,
      );
  ...
  const commitCurrentPathToState = () => {
    ...
    if (finalizedPath) {
      setInkPaths((prev) => [
        ...prev,
        {
          id: `ink-${Date.now()}-${Math.random().toString(36).substring(7)}`,
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>
        Ordonnance Numérique Intelligente (Smart Pen)
      </Text>
      ...
      <View style={styles.toolbar}>
        {hardwareError ? (
          ...
          <Text
            style={{
              color: isConnected ? 'green' : '#666',
              fontWeight: 'bold',
            }}
          >
            {isConnected
              ? '🟢 Capteur Optique Actif (Écriture...)'
              : '⚪ Stylo Déconnecté'}
          </Text>
        )}
        ...
        <Button
          title={
            isConnected ? "Clôturer l'ordonnance" : 'Appairer le Stylo WONDRx'
          }
