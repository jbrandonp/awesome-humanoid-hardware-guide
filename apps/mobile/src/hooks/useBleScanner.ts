...
export function useBleScanner(): BleScannerState & {
      ...
      manager.startDeviceScan(
        ...
        async (scanError: BleError | null, scannedDevice: Device | null) => {
          ...
          if (scannedDevice && scannedDevice.name) {
            console.log(
              `[BLE] Matériel médical détecté : ${scannedDevice.name} (${scannedDevice.id})`,
            );
            manager.stopDeviceScan();
            setIsScanning(false);
  ...
  const connectAndSubscribe = async (
    ...
    try {
      ...
      if (
        ...
        services.find((s) => s.uuid.includes(GATT_PROFILES.HEART_RATE.service))
      ) {
        targetService = GATT_PROFILES.HEART_RATE.service;
        targetChar = GATT_PROFILES.HEART_RATE.characteristic;
        vitalType = 'HEART_RATE';
      } else if (
        services.find((s) => s.uuid.includes(GATT_PROFILES.GLUCOSE.service))
      ) {
  ...
  const saveVitalDataOfflineSecurely = async (
    ...
    try {
      ...
      console.log(
        `[DB] Donnée ${vitalData.type} sauvegardée hors-ligne avec succès.`,
      );
      ...
      console.error(
        "[CRITICAL DB ERROR] Crash d'écriture WatermelonDB.",
        dbError,
      );
      setSystemError(
