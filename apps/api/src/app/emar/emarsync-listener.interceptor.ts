...
export class EmarsyncListener implements NestInterceptor {
  ...
  async intercept(context: ExecutionContext, next: CallHandler): Promise<Observable<unknown>> {
    ...
    try {
      // 1. Idempotency Check (Redis) - Fail-Open if Redis is down
      let isAlreadyProcessed = false;
      try {
        const cached = await this.redisClient.get(idempotencyKey);
        isAlreadyProcessed = !!cached;
      } catch (redisError) {
        this.logger.warn(`[EmarsyncListener] Redis is UNAVAILABLE. Skipping idempotency check: ${redisError}`);
      }
      ...
      // 3. Mark Idempotency in Redis - Fail-Open
      try {
        await this.redisClient.set(idempotencyKey, 'PROCESSED', 'EX', 86400);
      } catch (redisError) {
        this.logger.warn(`[EmarsyncListener] Failed to mark idempotency in Redis: ${redisError}`);
      }
