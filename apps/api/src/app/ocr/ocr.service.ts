...
export class OcrReviewRequiredException extends HttpException {
  constructor(public readonly extractedData: NerOcrResult) {
    super(
      {
        message: "REVIEW_REQUIRED: La qualité du scan est trop faible ou le terme médical est ambigu (Confiance < 95%). Veuillez valider manuellement la donnée avant insertion au dossier patient.",
...
export class OcrService {
  ...
  async extractVitalsWithNer(ocrText: string): Promise<NerOcrResult> {
    this.logger.log(`[OCR-NER] Analyse sémantique d'un document brut (${ocrText.length} caractères)...`);
    ...
    for (const entity of result.extractedEntities) {
       if (entity.confidenceScore < this.CLINICAL_CONFIDENCE_THRESHOLD) {
          this.logger.warn(`[OCR-NER] Rejet de sécurité: L'entité ${entity.category} a un score trop faible (${(entity.confidenceScore * 100).toFixed(1)}%). Validation humaine requise.`);
    ...
    this.logger.log(`[OCR-NER] Extraction terminée avec succès. ${result.extractedEntities.length} entités vitales certifiées à >95%.`);
    return result;
  }
   ...
   */
  private analyzeContextWindow(text: string, keywords: string[], expectedUnits: string[]): { value: number, unit: string | null, confidence: number, rawMatch: string } | null {
    ...
    for (const keyword of keywords) {
      ...
      const contextWindow = textLower.substring(keywordIndex, keywordIndex + 40);

      // Cherche une valeur numérique dans cette fenêtre (ex: " : 120 ")
      const valueMatch = contextWindow.match(/[\s:=]+([0-9]{2,3}(?:[.,][0-9])?)/);
      ...
      let confidence = 0.70; // Base score pour avoir trouvé le Mot Clé + Chiffre

      // Cherche l'unité de mesure juste après le chiffre
      const valueEndIndex = contextWindow.indexOf(valueMatch[1]) + valueMatch[1].length;
      const unitContext = contextWindow.substring(valueEndIndex, valueEndIndex + 15).trim();
      ...
      return {
         ...
         rawMatch: contextWindow.substring(0, valueEndIndex + (unitFound ? unitFound.length : 0)).trim()
      };
    }
