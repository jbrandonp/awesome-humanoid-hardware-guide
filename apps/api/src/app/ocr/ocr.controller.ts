...
export class OcrController {
  ...
  async extractVitals(
    ...
    if (!text || !patientId) {
      throw new HttpException('Texte brut et identifiant patient requis pour le moteur NLP.', HttpStatus.BAD_REQUEST);
    }
    ...
    try {
      ...
      if (nlpResult.isAutoInjectable || forceInjection) {
        ...
        for (const entity of nlpResult.extractedEntities) {
          switch (entity.category) {
             case 'GLUCOSE': glucose = entity.numericValue; break;
             case 'HEART_RATE': heartRate = entity.numericValue; break;
             case 'TEMPERATURE': temperature = entity.numericValue; break;
             case 'BLOOD_PRESSURE_SYSTOLIC': bloodPressure = `${entity.numericValue}/...`; break; // Simplification
          }
        }
      ...
      throw new HttpException('Aucune donnée vitale exploitable n\'a pu être extraite du rapport.', HttpStatus.UNPROCESSABLE_ENTITY);
       ...
       throw new HttpException('Échec du moteur sémantique.', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
}
