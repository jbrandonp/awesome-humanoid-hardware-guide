...
export class BioSensorService {
  ...
  processCameraPulseStream(payload: RppgVideoStreamPayload): BioSensorResult {
    this.logger.log(`[BioSensor] Analyse rPPG démarrée pour le patient ${payload.patientId} (Durée: ${payload.durationSeconds}s, FPS: ${payload.fps})`);

    try {
      // 1. VÉRIFICATION DES LIMITES EXTRÊMES (Error Handling & RAM Protection)
      if (!payload.greenChannelMeans || payload.greenChannelMeans.length === 0) {
        throw new HttpException("Le flux vidéo (Green Channel) est vide ou corrompu.", HttpStatus.BAD_REQUEST);
      }

      // Il faut au minimum 10 secondes de vidéo pour obtenir une fréquence cardiaque fiable
      if (payload.durationSeconds < 10.0 || payload.greenChannelMeans.length < (10 * payload.fps)) {
      ...
      const rawSignal = new Float64Array(payload.greenChannelMeans.slice(0, maxFrames));
      ...
      const mean = rawSignal.reduce((sum, val) => sum + val, 0) / rawSignal.length;
      ...
      const filteredSignal = this.applyBandpassFilter(detrendedSignal, payload.fps, this.MIN_HR_HZ, this.MAX_HR_HZ);
      ...
      if (peaks.length < 5) {
         ...
         this.logger.warn(`[BioSensor] Échec rPPG: Seulement ${peaks.length} battements détectés en ${payload.durationSeconds}s. Qualité vidéo insuffisante.`);
         return {
            status: 'POOR_SIGNAL_QUALITY',
            confidenceScorePercentage: 0,
            message: "Mouvements excessifs ou mauvaise luminosité. Veuillez stabiliser le doigt sur la caméra et recommencer l'acquisition."
         };
      }
      ...
      const variance = intervalsInSeconds.reduce((sum, val) => sum + Math.pow(val - avgInterval, 2), 0) / intervalsInSeconds.length;
      ...
      this.logger.log(`[BioSensor] Rythme estimé: ${estimatedBpm} BPM (Confiance: ${confidence.toFixed(1)}%, Pics détectés: ${peaks.length})`);
      ...
      if (confidence < 40) {
         return {
            ...
            message: "Rythme extrêmement irrégulier ou bruit optique. Résultat non fiable médicalement."
         };
      }
      ...
      this.logger.error(`[FATAL] Crash mathématique du moteur de Computer Vision.`, error);
      throw new HttpException(
   ...
   */
  private applyBandpassFilter(signal: Float64Array, fps: number, lowCutoffHz: number, highCutoffHz: number): Float64Array {
    ...
    for (let i = 0; i < signal.length; i++) {
      let sum = 0;
      let count = 0;
      for (let j = Math.max(0, i - lowPassWindowSize); j <= Math.min(signal.length - 1, i + lowPassWindowSize); j++) {
         sum += signal[j];
         count++;
      }
    ...
    for (let i = 0; i < tempLowPass.length; i++) {
      let sum = 0;
      let count = 0;
      for (let j = Math.max(0, i - highPassWindowSize); j <= Math.min(tempLowPass.length - 1, i + highPassWindowSize); j++) {
         sum += tempLowPass[j];
         count++;
      }
    ...
    for (let i = 1; i < signal.length - 1; i++) {
      ...
      if (signal[i] > signal[i - 1] && signal[i] > signal[i + 1]) {
         ...
         if (signal[i] > peakThreshold) {
            ...
            if (peaks.length === 0 || (i - peaks[peaks.length - 1]) >= minDistanceBetweenPeaks) {
               peaks.push(i);
