...
export class ClinicalRecordService {
  ...
  constructor(
    ...
    private recordModel: Model<ClinicalRecordDocument>,
  ) {}

  async createSpecialtyRecord(
    patientId: string,
    specialty: string,
    formData: any,
  ) {
    this.logger.log(
      `Création d'un dossier ${specialty} dynamique pour le patient ${patientId}`,
    );
  ...
  async getPatientRecords(patientId: string) {
    ...
    return this.recordModel
      .find({ patientId, deletedAt: { $exists: false } })
      .exec();
  }
  ...
  async deleteRecord(recordId: string) {
    this.logger.log(
      `Suppression logique (Tombstone) du record MongoDB ${recordId}`,
    );
  ...
  async hardDeletePatientRecords(patientId: string): Promise<number> {
    this.logger.warn(
      `[DPDPA] PURGE DÉFINITIVE de tous les dossiers dynamiques MongoDB pour le patient ${patientId}`,
    );
