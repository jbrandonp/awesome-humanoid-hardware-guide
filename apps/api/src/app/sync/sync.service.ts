...
export class SyncService {
  ...
  async pushChanges(changes: any) {
    ...
    if (changes.patients) {
      ...
      const patientUpdates = changes.patients.updated.map((patient: any) =>
    ...
    if (changes.prescriptions) {
      if (changes.prescriptions.created.length > 0) {
        await this.prisma.prescription.createMany({
          data: changes.prescriptions.created.map((prescription: any) => ({
            ...
            crdtAdministrations: prescription.administrations
              ? Buffer.from(prescription.administrations, 'base64')
              : null,
            status: 'synced',
          })),
          skipDuplicates: true,
      ...
      for (const prescription of changes.prescriptions.updated) {
        ...
        if (prescription.administrations) {
          const clientUpdate = Buffer.from(
          ...
          if (existingPrescription?.crdtAdministrations) {
            try {
              ...
              this.logger.error(
                `Error loading existing Yjs state for prescription ${prescription.id}`,
                e,
              );
            }
          }
          ...
          try {
            ...
            if (conflictDetected) {
              this.logger.warn(
                `CRDT Conflict (eMAR): Potential overdose detected for prescription ${prescription.id}`,
              );
              ...
              throw new Error(
                `CRDT Conflict: Potential overdose for ${prescription.id}. Skipping sync update to protect patient.`,
              );
            }

            mergedAdministrationsBuffer = Buffer.from(
              Y.encodeStateAsUpdate(serverDoc),
            );
          } catch (e) {
            this.logger.error(
              `Conflict resolving Yjs document for prescription ${prescription.id}`,
              e,
            );
          }
    ...
    if (changes.vitals) {
      ...
      const vitalUpdates = changes.vitals.updated.map((vital: any) =>
