  ...
  async pushChanges(changes: any) {
    ...
    if (changes.visits) {
      ...
      if (changes.visits.updated.length > 0) {
        ...
        const updates = changes.visits.updated.map((visit: any) => {
          ...
          let mergedNotesBuffer = existingVisit?.notes || null;
    ...
    if (changes.prescriptions) {
      ...
      if (changes.prescriptions.updated.length > 0) {
        ...
        const existingPrescriptions = await this.prisma.prescription.findMany({
        ...
        for (const prescription of changes.prescriptions.updated) {
          ...
          let mergedAdministrationsBuffer = existingPrescription?.crdtAdministrations || null;
          ...
          updates.push(this.prisma.prescription.update({
