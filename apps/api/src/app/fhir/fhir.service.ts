...
export const FhirPatientSchema = z.object({
  ...
  birthDate: z
    ...
    .regex(
      /^\d{4}-\d{2}-\d{2}$/,
      'Date de naissance non conforme (YYYY-MM-DD)',
    ),
...
export class FhirService {
  ...
  async exportPatientToFhir(patientId: string): Promise<FhirBundle> {
    this.logger.log(
      `[FHIR Export] Extraction complète du dossier patient: ${patientId}`,
    );
    ...
    try {
      ...
      throw new UnprocessableEntityException(
        "Données d'identité du patient corrompues. Le nom ou la date de naissance sont manquants.",
      );
    }
      ...
      if (record.specialty === 'PEDIATRICS' && record.data?.headCircumference) {
        entries.push({
          ...
          resource: {
            ...
            valueQuantity: {
              value: record.data.headCircumference,
              unit: 'cm',
              system: 'http://unitsofmeasure.org',
              code: 'cm',
            },
    ...
    if (!validationResult.success) {
      this.logger.error(
        `[FATAL FHIR ERROR] Le sérialiseur a généré un document invalide (Violation Schema R4).`,
        validationResult.error.format(),
      );
      throw new UnprocessableEntityException(
        'Impossible de générer un dossier médical légal : données patient incomplètes ou formatées de manière incorrecte.',
      );
    }

    this.logger.log(
      `[FHIR Export] Succès. Bundle FHIR R4 généré avec ${validationResult.data.entry.length} entrées.`,
    );
   ...
   */
  async getActivePrescriptionsForPharmacy(
    patientId: string,
  ): Promise<FhirBundle> {
    ...
    if (!validationResult.success) {
      this.logger.error(
        'Erreur de formatage FHIR pour la pharmacie locale.',
