...
export class FhirService {
  ...
  async exportPatientToFhir(patientId: string): Promise<FhirBundle> {
    ...
    for (const vital of patient.vitals) {
      ...
      if (vital.bloodPressure !== null && vital.bloodPressure !== undefined) {
        entries.push({
    ...
    if (!validationResult.success) {
      this.logger.error(
        `[FATAL FHIR ERROR] Le sérialiseur a généré un document invalide.`,
  ...
  async ingestObservation(fhirPayload: unknown): Promise<FhirObservation> {
    ...
    const createdVital = await this.prisma.vital.create({
      data: {
        ...dbVitalData,
        recordedAt: parsedData.effectiveDateTime
          ? new Date(parsedData.effectiveDateTime)
          : new Date(),
      },
    });

    const code = parsedData.code?.coding?.[0]?.code;
    let type: 'temperature' | 'heartRate' | 'bloodPressure' | 'glucose' =
      'temperature';
