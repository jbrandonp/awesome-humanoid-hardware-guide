...
export class DrugInteractionService {
  ...
  async overridePrescription(dto: DrugInteractionOverrideDto): Promise<{ success: boolean; report: InteractionReport }> {
    ...
    // Validate override justification logic
    const hashedJustification = crypto.createHash('sha256').update(dto.justification).digest('hex');
    ...
    await this.prisma.auditLog.create({
      data: {
        ...
        metadata: {
          ...
          justificationHash: hashedJustification,
        },
      },
    });
