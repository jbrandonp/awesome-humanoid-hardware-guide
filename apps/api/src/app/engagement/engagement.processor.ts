...
export class EngagementProcessor extends WorkerHost {
  ...
  async process(job: Job<SendWhatsappJobData>): Promise<void> {
    this.logger.log(`[Worker] Traitement du Job n°${job.id} (Notification ${job.data.notificationId})...`);
    ...
    try {
      ...
      this.logger.log(`[Worker] Message remis à l'API Meta avec succès. WAMID: ${metaMessageId}`);

    } catch (httpError: unknown) {
      this.logger.error(`[FATAL API META] L'appel HTTP vers WhatsApp Business a échoué (Tentative ${job.attemptsMade + 1}/3).`, httpError);

      // Si la tâche a atteint sa limite de tentatives imposée par Bull
      if (job.attemptsMade >= 2) {
         // 3. FALLBACK SMS CLASSIQUE
         this.logger.warn(`[FALLBACK SMS] L'API WhatsApp est définitivement hors-ligne. Basculement sur la passerelle SMS d'urgence pour le patient ${job.data.patientPhone}.`);
  ...
  private async triggerSmsFallback(data: SendWhatsappJobData): Promise<void> {
     try {
       ...
       this.logger.error(`[Worker SMS] Échec du fallback SMS. Le message ne peut pas être délivré.`, fallbackError);
       await this.prisma.notificationStatus.update({
         where: { id: data.notificationId },
