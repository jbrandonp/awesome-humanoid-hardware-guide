...
export class PatientEngagementService {
  ...
  async sendVaccinationReminder(
    payload: EngagementMessagePayload,
  ): Promise<EngagementResult> {
    this.logger.log(
      `[WhatsApp Business] Préparation du rappel de vaccination (${payload.vaccineName}) pour le patient ${payload.patientId}.`,
    );
    ...
    try {
        ...
        await tx.auditLog.create({
          data: {
            ...
            metadata: {
              ...
              targetPhone: payload.patientPhone,
            },
          },
        });
      ...
      return {
        ...
        notificationId: notification.id,
        message:
          "Le rappel a été placé dans la file d'attente sécurisée (Redis) et sera envoyé sous peu.",
      };
    } catch (error: unknown) {
      this.logger.error(
        `[FATAL] Impossible de sécuriser le rappel dans la base ou la file Redis.`,
        error,
      );
      return {
        status: 'ERROR',
